...
def draw_polygons( matrix, screen, color ):
    ...
    while point < len(matrix) - 1:
        if bfc and dot_product(calculate_normal(matrix, point), [0, 0, 1]) < 0:
            pass
        else:
    ...
    for i in range(m):
        if i % n == 0:
            add_polygon(polygons, points[i][0], points[i][1], points[i][2],
                                  points[(i+1) %m][0], points[(i+1) %m][1], points[(i+1) %m][2],
                                  points[(i+n+1) %m][0], points[(i+n+1) %m][1], points[(i+n+1) %m][2])
        elif i % n == n - 2:
            add_polygon(polygons, points[i][0], points[i][1], points[i][2],
                                  points[(i+n+1) %m][0], points[(i+n+1) %m][1], points[(i+n+1) %m][2],
                                  points[(i+n) %m][0], points[(i+n) %m][1], points[(i+n) %m][2])
        elif i % n != n - 1:
            add_polygon(polygons, points[i][0], points[i][1], points[i][2],
                                  points[(i+1) %m][0], points[(i+1) %m][1], points[(i+1) %m][2],
                                  points[(i+n+1) %m][0], points[(i+n+1) %m][1], points[(i+n+1) %m][2])
...
def add_torus(polygons, cx, cy, cz, r0, r1, steps ):
    ...
    for i in range(m):
        if i % (n-1) == (n - 2):
            ...
            add_polygon(polygons, points[i][0], points[i][1], points[i][2],
                                  points[(i+1) %m][0], points[(i+1) %m][1], points[(i+1) %m][2],
                                  points[(i-n+2) %m][0], points[(i-n+2) %m][1], points[(i-n+2) %m][2])
        else:
            add_polygon(polygons, points[i][0], points[i][1], points[i][2],
                                  points[(i+n-1) %m][0], points[(i+n-1) %m][1], points[(i+n-1) %m][2],
                                  points[(i+n) %m][0], points[(i+n) %m][1], points[(i+n) %m][2])
